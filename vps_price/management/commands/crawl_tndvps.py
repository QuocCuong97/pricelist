...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        def new_pack_125():
            lst = get_pack_125()
            new_object = Vps.objects.update_or_create(vendor=Vendor.objects.get(name='TND'), pack=lst[0], defaults = {'vcpu': lst[1], 'ssd': lst[2], 'ram': lst[3], 'price_1': lst[4], 'price_3': lst[5], 'price_6': lst[6], 'price_12': lst[7], 'link': lst[8]})
        def new_pack_250():
            lst = get_pack_250()
            new_object = Vps.objects.update_or_create(vendor=Vendor.objects.get(name='TND'), pack=lst[0], defaults = {'vcpu': lst[1], 'ssd': lst[2], 'ram': lst[3], 'price_1': lst[4], 'price_3': lst[5], 'price_6': lst[6], 'price_12': lst[7], 'link': lst[8]})
        def new_pack_500():
