...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        def new_com():
            lst = get_com()
            new_object = Domain.objects.update_or_create(vendor=Vendor.objects.get(name='Domain.com'), domain_type='com', defaults = {'reg_origin': lst[0], 'reg_promotion': lst[1], 'renew_price': lst[2], 'reg_origin_usd': lst[3], 'reg_promotion_usd': lst[4], 'renew_price_usd': lst[5]})
        def new_net():
            lst = get_net()
            new_object = Domain.objects.update_or_create(vendor=Vendor.objects.get(name='Domain.com'), domain_type='net', defaults = {'reg_origin': lst[0], 'reg_promotion': lst[1], 'renew_price': lst[2], 'reg_origin_usd': lst[3], 'reg_promotion_usd': lst[4], 'renew_price_usd': lst[5]})
        def new_org():
            lst = get_org()
            new_object = Domain.objects.update_or_create(vendor=Vendor.objects.get(name='Domain.com'), domain_type='org', defaults = {'reg_origin': lst[0], 'reg_promotion': lst[1], 'renew_price': lst[2], 'reg_origin_usd': lst[3], 'reg_promotion_usd': lst[4], 'renew_price_usd': lst[5]})
        def new_info():
            lst = get_info()
            new_object = Domain.objects.update_or_create(vendor=Vendor.objects.get(name='Domain.com'), domain_type='info', defaults = {'reg_origin': lst[0], 'reg_promotion': lst[1], 'renew_price': lst[2], 'reg_origin_usd': lst[3], 'reg_promotion_usd': lst[4], 'renew_price_usd': lst[5]})
        if kwargs['com']:
            new_com()
        elif kwargs['net']:
            new_net()
